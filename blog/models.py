...
class BlogPost(models.Model):
    ...
    def get_related_posts(self, limit=3):
        ...
        # Build query conditions for PostgreSQL deployment
        conditions = Q(status='published') & ~Q(id=self.id)
        
        # Prioritize posts from same category or with similar tags
        if self.category or self.tags.exists():
            category_condition = Q(category=self.category) if self.category else Q()
            tag_condition = Q(tags__in=self.tags.all()) if self.tags.exists() else Q()
            
            # Combine conditions with OR
            if self.category and self.tags.exists():
                conditions &= (category_condition | tag_condition)
            elif self.category:
                conditions &= category_condition
            elif self.tags.exists():
                conditions &= tag_condition
        
        return BlogPost.objects.filter(conditions).distinct().order_by('-published_date')[:limit]
