/**
 * Triple G BuildHub - Site Diary Draft Management JavaScript
 * Handles draft functionality including:
 * - Loading diary drafts from localStorage
    ...
    if (viewBtns.length && cardView && tableView) {
        viewBtns.forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                localStorage.setItem('preferredDiaryDraftsView', viewType);
            });
        });
        
        // Load saved view preference
        const savedView = localStorage.getItem('preferredDiaryDraftsView');
        if (savedView) {
...
function loadDrafts() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
        ...
        // Check if key matches the format for diary drafts
        if (key.startsWith('diaryDraft_')) {
            try {
                const draft = JSON.parse(localStorage.getItem(key));
                draft.id = key.replace('diaryDraft_', ''); // Extract ID from key
                drafts.push(draft);
            } catch (e) {
        ...
        drafts.forEach(draft => {
            ...
            row.innerHTML = `
                ...
                <td class="table-actions">
                    ...
                    <button class="action-btn submit-btn" title="Submit Entry for Review" data-id="${draft.id}">
...
function setupCardActions(card, draft) {
    ...
    if (editBtn) {
        editBtn.addEventListener('click', () => {
            window.location.href = `diary.html?edit=${draft.id}`;
        });
    }
...
function showPreviewModal(draft) {
    ...
    document.getElementById('preview-reading-time').textContent = draft.readingTime || '3';
    ...
    if (previewImage) {
        if (draft.featuredImage) {
            ...
            // Use a default site image
            previewImage.src = '/static/images/image1.jpg';
            previewImage.style.display = 'block';
        }
    }
...
function showNotificationModal() {
    ...
    if (modal) {
        ...
        // Set up OK button
        const okBtn = modal.querySelector('.modal-confirm');
        if (okBtn) {
            okBtn.onclick = () => {
...
function deleteDraft(draftId) {
    ...
    localStorage.removeItem(`diaryDraft_${draftId}`);
    ...
    // Show notification
    showToast('Diary draft deleted successfully');
}
...
function submitDraft(draftId) {
    ...
    const draftKey = `diaryDraft_${draftId}`;
    ...
    // Show notification
    showToast('Diary entry submitted for supervisor review');
}
...
function initModalHandlers() {
    ...
    const closeButtons = document.querySelectorAll('.close-modal, #preview-close-btn, #delete-cancel-btn, #submit-cancel-btn');
